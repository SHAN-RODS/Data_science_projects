...
class BuildingAnalysis(BaseModel):
    ...
    description=(
        ...
        "The first must always be the base case with all exits available. "
        "The remaining scenarios must be selected autonomously based on the "
        "building geometry, exit arrangement, storeys, occupancy distribution, "
        "travel distances and potential evacuation challenges."
    )
)
...
_SYSTEM = (
    ...
    3) SCENARIOS
    You must create AT LEAST FOUR distinct evacuation scenarios.

    The scenarios must be generated autonomously from the building geometry,
storey layout, space types, occupant distribution, exit locations,
travel distances and circulation network.

Do NOT choose scenarios randomly.

Instead analyse the building and create scenarios that are meaningful for
this specific IFC model.

The first scenario MUST always be:

• Base Case
    - all final exits available
    - normal occupancy

The remaining scenarios should be chosen by identifying the most realistic
or most challenging evacuation conditions for this building.

Examples include (do not treat these as mandatory):

• loss of the busiest exit
• loss of the exit serving the largest population
• loss of an upper-floor escape route
• night occupancy
• daytime peak occupancy
• maintenance closure of one exit
• reduced exit capacity
• evacuation during high occupancy on one storey
• simultaneous congestion at a stair
• any geometry-specific evacuation challenge discovered from the IFC

Choose whichever scenarios best stress the evacuation routes.

Each scenario must be substantially different from the others.

For every scenario determine yourself:
...
The occupants_total must be consistent with the occupant_distribution.

Every number mentioned in the narrative must already exist in the structured fields.
