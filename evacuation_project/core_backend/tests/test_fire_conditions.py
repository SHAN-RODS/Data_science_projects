...
def sample_fire(affected_exits=()):
...
def sample_object(discounted=(), affected=(), with_fire=True):
    ...
    if with_fire:
        obj["scenarios"][0]["fire_conditions"] = sample_fire(affected)
    return obj


def test_a_fire_that_explains_the_discounted_exit_passes():
    assert fire_condition_issues(sample_object(discounted=["Exit 1"], affected=["Exit 1"])) == []


def test_a_scenario_with_no_fire_at_all_passes_when_nothing_is_discounted():
    assert fire_condition_issues(sample_object()) == []


def test_a_missing_fire_block_is_flagged():
    issues = fire_condition_issues(sample_object(with_fire=False))
    assert len(issues) == 1
    assert issues[0]["field"] == "fire_conditions"


def test_an_exit_discounted_for_no_stated_reason_is_flagged():
    issues = fire_condition_issues(sample_object(discounted=["Exit 1"], affected=[]))
...
def test_an_exit_the_fire_hits_but_the_scenario_still_uses_is_flagged():
    ...
    issues = fire_condition_issues(sample_object(discounted=[], affected=["Exit 1"]))
...
def test_both_directions_are_reported_together():
    issues = fire_condition_issues(sample_object(discounted=["Exit 1"], affected=["Exit 2"]))
    assert len(issues) == 1
...
def test_a_fire_naming_an_exit_that_does_not_exist_is_caught():
    ...
    obj = sample_object(discounted=["Exit 9"], affected=["Exit 9"])
    assert unknown_exit_references(obj) == [{"scenario": "SCN-001", "unknown_exits": ["Exit 9"]}]


def test_validate_reports_the_fire_invariant():
    obj = minimal_object("Evacuate 10 occupants.")            # carries no fire_conditions at all
